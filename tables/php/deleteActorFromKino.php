...
include("../../php/connect_mssql_db.php");
 ...
 $result = sqlsrv_query($conn,"exec delActorFromKino {$_POST["id"]}, $name;");
 if ($result){
  	  $result = sqlsrv_query($conn,"exec getActorForKinoById {$_POST["id"]};");
	  if (!$result){
	  ...
	  while($row = sqlsrv_fetch_array($result)){
	    ...
	    $genres.='<tr><td style="width:50px"><button onclick="deleteActor(\''.$item.'\','.$_POST["id"].')" ><i class="icon-remove"></button></td><td>'.$item.'</td></tr>';
	  }
	  $genres.=" <tr>
	                            <td><button onclick='addActor({$_POST["id"]})' class='btn btn-success' > <span>Добавить</span> </button></td>
