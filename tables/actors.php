...
include("../php/connect_mssql_db.php");
  ...
  while($row = sqlsrv_fetch_array($result)){
        ...
        $content .= '<tr id="row'.$i.'">
                            <td style="width:50px"><button onclick="updateActor(\''.$row[0].'\',\''.$i.'\')"><i class="icon-pencil"></button></td>
                            <td style="width:50px"><button onclick="deleteActor(\''.$row[0].'\')"><i class="icon-remove"></button></td>
                            <td>'.$row[0].'</td>
  ...
  $content.="<tr>
                <td colspan='2'><button class='btn btn-success' onclick='addActor()'> <span>Добавить</span> </button></td>
...
<html>
  <head>
    <meta charset="utf-8">
    <title>Кинокаталог - Таблица "Актеры"</title>
    ...
    function addActor(){
      ...
      if(!isEmpty(name) && isValidDate(date) && !isEmpty(country) && !isEmpty(gender)){
        $.post("php/addActor.php",{"name":name,"date":date,"country":country,"gender":gender},function(data){
          if(data!="error") document.location.href="actors.php";
    ...
    function deleteActor(name){
      $.post("php/deleteActor.php",{"name":name},function(data){
            if(data!="error") document.location.href="actors.php";
            else alert("Ошибка удаления");;
        });
    }
    function updateActor(actor,i){
       ...
       var gender = elem[5].innerText;
            $(elem[0]).replaceWith('<td style="width:50px"><button onclick="saveActor(\''+actor+'\',\''+i+'\')"><i class="icon-ok"></button></td>');
    ...
    function saveActor(actor,i){
       var name = $("#name"+i).val();
       var date = $("#date"+i).val();
       var country = $("#country"+i).val();
       var gender = $("#gender"+i).val();
      if(!isEmpty(name) && isValidDate(date) && !isEmpty(country) && !isEmpty(gender)){
       $.post("php/updateActor.php",{"name":name,"date":date,"country":country,"gender":gender,"actor":actor},function(data){
          if(data!="error") document.location.href="actors.php";
            else alert("Ошибка добавления");
        })
      }
