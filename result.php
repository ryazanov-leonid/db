...
if($_SESSION["login"]){
  ...
  if(!empty($_GET['name'])){
    $pages=ceil(search($conn,$_GET['name'])/$items);
    if(empty($pages)){
      $content="<div style='margin:5px; color:red; margin-bottom:10px;'><strong>Ничего не найдено</strong></div>";
    }
    else {
      include('php/pagination.php');
      $result = getResultSearch($conn,$start,$end,$_GET['name']);
...
<html>
  <head>
    <meta charset="utf-8">
    <title>Кинокаталог - Результат поиска</title>
